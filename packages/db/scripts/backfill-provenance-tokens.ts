...
// Default targets support-tenant; pass an org slug as argv[2] for another tenant,
...
// against a live count of ~4,247 tokens for the current support-tenant prompt).
...
async function main() {
  const arg = process.argv[2] ?? 'support-tenant';
  const orgFilter =
    arg === 'ALL'
      ? {}
